...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'allenbh_ContigFilter'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from allenbh_ContigFilter.allenbh_ContigFilterImpl import allenbh_ContigFilter
impl_allenbh_ContigFilter = allenbh_ContigFilter(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['allenbh_ContigFilter.filter_contigs_async'] = ['allenbh_ContigFilter', 'filter_contigs']
async_check_methods['allenbh_ContigFilter.filter_contigs_check'] = ['allenbh_ContigFilter', 'filter_contigs']
sync_methods['allenbh_ContigFilter.filter_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'allenbh_ContigFilter'
        ...
        self.rpc_service.add(impl_allenbh_ContigFilter.filter_contigs,
                             name='allenbh_ContigFilter.filter_contigs',
                             types=[dict])
        self.method_authentication['allenbh_ContigFilter.filter_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "allenbh_ContigFilter but no authentication header was passed"
                            raise err
